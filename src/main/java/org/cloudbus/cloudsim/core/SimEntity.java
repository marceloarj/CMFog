 ...
 * <li> {@link #shutdownEntity()} is invoked by the {@link Simulation} before
 * the simulation finishes. If you want to save data in log files this is the
 * method in which the corresponding code would be placed.
 * </ul>
 *
...
public abstract class SimEntity implements Cloneable {
    ...
    protected void send(int entityId, double delay, int cloudSimTag, Object data) {
        ...
        int srcId = getId();
        boolean isPartOfMD = SimulationEntityHelper.isPartOfMD(getId(), entityId);
        if ((entityId != srcId) && !isPartOfMD) {// does not delay self messages
//            if (cloudSimTag == FogEvents.TUPLE_ARRIVAL) {
//                System.out.println("[" + getName() + " -> " + CloudSim.getEntityName(entityId) + "] = " + (delay + getNetworkDelay(srcId, entityId)) + " (ND: " + getNetworkDelay(srcId, entityId) + ")");
//            }
    ...
    protected double getNetworkDelay(int src, int dst) {
        if (NetworkTopology.isNetworkEnabled()) {
            if (this instanceof MobileDevice) {
                MobileDevice aMobileDevice = (MobileDevice) this;
                return aMobileDevice.getUplinkLatency();
            } else if (CloudSim.getEntity(dst) instanceof MobileDevice) {
                MobileDevice aMobileDevice = (MobileDevice) CloudSim.getEntity(dst);
                return aMobileDevice.getUplinkLatency();
            }
