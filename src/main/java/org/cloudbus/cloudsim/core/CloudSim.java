...
public class CloudSim {
     ...
     * @return true, if successful otherwise
     */
    public static double lastTick = -1;

    public static boolean runClockTick() {
        SimEntity ent;
        boolean queue_empty;
        int entities_size = entities.size();
        if (CloudSim.clock() % 1000 == 0 && lastTick != (CloudSim.clock() / 1000)) {
            System.out.println("====================== Clock(CS): " + CloudSim.clock() + " ======================");
            lastTick = (CloudSim.clock() / 1000);
        }
//		int m=0;
