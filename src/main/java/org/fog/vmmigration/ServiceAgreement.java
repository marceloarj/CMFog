...
public class ServiceAgreement {
    ...
    public static boolean serviceAgreement(FogDevice serverCloudlet, MobileDevice smartThing) {
        ...
        if (!HcheckLinkStatus(smartThing.getVmLocalServerCloudlet(), serverCloudlet)) {
    ...
    public static boolean HcheckLinkStatus(FogDevice sourceServerCloudlet, FogDevice destinationServerCloudlet) {
        if (isConnectedtoCloud(sourceServerCloudlet) && isConnectedtoCloud(destinationServerCloudlet)) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isConnectedtoCloud(FogDevice aFogDevice) {
        if (aFogDevice.getParentId() == CMFog.cloud.getId()) {
            return true;
        }
        FogDevice L2check = CMFogHelper.getCloudletById(aFogDevice.getParentId(), CMFog.cloudletsL2);
        if (L2check != null) {
            return isConnectedtoCloud(L2check);
        }
        return false;
    }
