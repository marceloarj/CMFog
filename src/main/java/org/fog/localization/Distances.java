...
public class Distances {
    ...
    public static int theClosestAp(List<ApDevice> apDevices, MobileDevice smartThing) {//verify what return type is better (int or ApDevice)		
        ApDevice choose = apDevices.get(0);//Id of the first ap in the list
        ...
        for (ApDevice ap : apDevices) {
            ...
            if (getDistance() < min) {
                choose = ap;//index or id
                min = getDistance();
            }
        }

        if (LocationHelper.isCovered(choose, smartThing)) {
            return choose.getMyId();//id
        } else {
            return -1;//flag error
        }
    }
    ...
    public static FogDevice closestUpperCloudlet(List<FogDevice> upperCloudlets, FogDevice cloudlet) {
        FogDevice choose = upperCloudlets.get(0);
        double min = checkDistance(choose.getCoord(), cloudlet.getCoord());

        for (FogDevice anAlternative : upperCloudlets) {
            setDistance(checkDistance(anAlternative.getCoord(), cloudlet.getCoord()));
            //if(getDistance() != 0)
            if (getDistance() < min) {
                choose = anAlternative;//id
                min = getDistance();
            }
        }
